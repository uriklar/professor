import { createContext, Dispatch, ReactChild, ReactChildren, useContext, useEffect, useMemo, useReducer } from 'react';
import { IAnswer, IBoard, ActionMap, IItem, AnswerState, IClues, ILikes, IUser } from '../types';
import {
	difference,
...
function reducer(state: IState, action: TActions) {
	...
	switch (action.type) {
		...
		case Actions.FoundConnection:
			nextAnswers = [...state.answers, { categoryId: action.payload.categoryId, state: AnswerState.Matched }];
			...
			if (nextAnswers.length === 3) {
				...
				nextAnswers = [...nextAnswers, { categoryId: missingCategoryId, state: AnswerState.Matched }];
			}
		...
		case Actions.FoundAnswer:
			...
			nextAnswers = state.answers.map((answer) =>
				answer.categoryId === categoryId ? { categoryId, state: AnswerState.Answered } : answer
			);
			setLocalStorage(state.boardId, nextAnswers);
			return {
				...state,
...
export default function Store({ children, board, ids, likes }: Props) {
	...
	const value = useMemo(() => ({ state, dispatch, board, ids, likes }), [board, ids, likes, state]);
	...
	useEffect(() => {
		if (state.selection.length === 4) {
			...
			if (connectionCategory) {
				...
				setTimeout(() => dispatch({ type: Actions.ResetSelection, payload: {} }), 150);
			}
		}
